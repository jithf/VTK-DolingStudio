...
int main(int argc, char*argv[])
{
	...
	iren->SetDesiredUpdateRate(3);  // 设置期望的更新速率为每秒3帧（3Hz）

	vtkSmartPointer<vtkStructuredPointsReader> reader = vtkSmartPointer<vtkStructuredPointsReader>::New();
	reader->SetFileName("C:\\Users\\luhy\\Desktop\\data\\ironProt.vtk");

	//  创建阈值过滤器
	vtkSmartPointer<vtkThreshold> thresh = vtkSmartPointer<vtkThreshold>::New();
	thresh->SetUpperThreshold(80);
	thresh->AllScalarsOff();  // 只使用活动标量数组进行阈值处理, 避免对所有标量数组应用阈值操作
	...
	volumeProperty->SetColor(colorTransferFunction);  //设置不透明度传输函数
	volumeProperty->SetScalarOpacity(opacityTransferFunction);  //设置颜色传输函数
	...
	{
		// 在 VTK 9.3.1 中，vtkHAVSVolumeMapper 已被完全移除且不再支持
