...
int main(int argc, char* argv[])
{
	...
	changer->SetOutputOrigin(100, 100, 0);  // 图像原点
	changer->SetOutputSpacing(5,5,1);  // 像素间隔
	changer->SetCenterImage(1);  // 将图像的原点置于图像中心
	changer->Update();

	changer->GetOutput()->GetDimensions(dims);
	std::cout<<"修改后图像维数:" <<dims[0]<<" "<<dims[1]<<" "<<dims[2]<<std::endl;
	changer->GetOutput()->GetOrigin(origin);
	std::cout<<"修改后图像原点:" <<origin[0]<<" "<<origin[1]<<" "<<origin[2]<<std::endl;  // 相对于原始原点的平移量
