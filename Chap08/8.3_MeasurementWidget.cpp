...
int main(int argc, char* argv[])
{
	...
	std::string fileName( "C:\\Users\\luhy\\Desktop\\data\\lena.bmp" );
	int widgetType = 1; // 0/1/2
	...
	if ( widgetType == 0 )
	{
		...
		distanceWidget->SetInteractor(renderWindowInteractor);  // 设置渲染窗口交互器
		distanceWidget->CreateDefaultRepresentation(); // 创建默认的几何实体 即十字形代表端点 端点之间用线相连
		static_cast<vtkDistanceRepresentation *>(distanceWidget->GetRepresentation())
			->SetLabelFormat("%-#6.3g px");  // 设置文本样式

		renderWindowInteractor->Initialize();
		renderWindow->Render();
		distanceWidget->On(); //激活widget实例
	...
	if ( widgetType == 2 )
	{
		...
		// 自定义的observer
		vtkSmartPointer<vtkBiDimensionalCallback> biDimensionalCallback =  vtkSmartPointer<vtkBiDimensionalCallback>::New();
